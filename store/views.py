...
@login_required#檢測使用者是否登入，若以登入才會執行add_to_cart()的程式
...
def checkout(request):
    ...
    if request.method == 'POST':
        #模擬貨到付款,假設訂單已經成功
        order=Order.objects.creat(user=request.user,total_price=total_price)
        for item in cart.cartitem_set.all():
            OrderItem.objects.creat(order=order,product=item.product,quantity=item.quantity)
        cart.cart_item_set.all().delete()
